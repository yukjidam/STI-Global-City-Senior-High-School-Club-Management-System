// === Adviser notifications (original popover style; text-only) ===
(() => {
  "use strict";
  if (window.__NOTIF_ADVISER__) return;
  window.__NOTIF_ADVISER__ = true;
  ...
  function renderOriginal(listEl, items) {
    ...
    for (const n of items) {
      ...
      if (n.kind === "membership_request") {
        titleText = "New membership request";
      } else if (n.kind === "club_event") {
        titleText = "New event this Friday";
        if (!bodyText) bodyText = "A club posted an announcement.";
      ...
      const url = n.url || "";
      const html = `
        <div class="notif-item" role="listitem" ${url ? `data-href="${url}"` : ""}>
  ...
  async function startWithDOM({ bell, pop, list, badge }) {
    ...
    const uid = me?.adviser_id || me?.id || null;
    const qs  = "role=adviser" + (uid ? `&uid=${encodeURIComponent(uid)}` : "");
    const URLS = {
      ...
      if (willOpen) {
        ...
        try {
          ...
          await getJSON(URLS.mark, {
            ...
            body: JSON.stringify({ role: "adviser", action: "mark_all_seen", uid }),
          });
          badge.hidden = true;
        } catch {}
      } else {
