...
async function __meOnly_ifNoQuery() {
  ...
  const base = PROJECT_BASE || '';
  if (target.kind === 'org') {
    window.location.replace(`${base}/Adviser-ClubDetails.html?org_id=${encodeURIComponent(target.id)}&org_name=${encodeURIComponent(target.name)}`);
  } else {
    window.location.replace(`${base}/Adviser-ClubDetails.html?id=${encodeURIComponent(target.id)}`);
  }
  return true;
}
...
async function __forceMineWhenSidebar() {
  ...
  if (me.role === 'student') {
    ...
    if (target?.id && String(target.id) !== String(currentId)) {
      location.replace(`${base}Adviser-ClubDetails.html?id=${encodeURIComponent(target.id)}`);
      return true;
    }
    return false;
  }
  ...
  if (me.role === 'adviser') {
    ...
    if (target?.kind === 'org') {
      ...
      location.replace(`${base}Adviser-ClubDetails.html?${qs}`);
      return true;
    }
...
async function loadOrgClubs(orgId, orgName){
  ...
  ul.innerHTML = list.slice(0, 6).map(c => {
    ...
    return `
      <li>
        <a class="suggest-item" href="Adviser-ClubDetails.html?id=${c.id}">
