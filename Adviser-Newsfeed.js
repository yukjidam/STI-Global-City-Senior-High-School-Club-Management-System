...
const BAD_WORDS = [
  ...
  "manyakis","manyak","malibog","syet","shyet","shet","pakshet","pakshit","paksyet",
  "yawa","piste","pisti",
  "rape","rapist","raping" // high-severity (fuzzy)
];

// 2) Canon + tolerant patterns (stars/leet/spacing/repeats)
const HIGH_SEVERITY = new Set(["rape","rapist","raping"]);
function canon(s=""){
...
const LEET = { a:"[a@4]", e:"[e3]", i:"[i1!|]", o:"[o0]", u:"[uuvvx]", s:"[s$5]", t:"[t7+]", b:"[b8]", g:"[g9]", c:"[c(\\[{xk]]", k:"[kqxc]" };
...
function wordPattern(w){
  const tokens = w.toLowerCase().split("").map(ch=>{
    ...
    const vowelSwap = VOWELS.has(ch) ? `|(?:[^a-z0-9]|_){1,10}` : ""; // **** can replace vowels
...
function listProfanities(text=""){
  ...
  for (let i=0;i<BAD_WORDS.length;i++) if (BAD_PATTERNS[i].test(s)) hits.push(BAD_WORDS[i]);
  const c = canon(s).replace(/[^a-z0-9]+/g," ").trim();
  if (c) {
    const toks = c.split(/\s+/);
    for (const tok of toks) for (const w of HIGH_SEVERITY)
      if (levenshtein(tok,w) <= 1) { hits.push(w); break; }
  }
  return [...new Set(hits)];
}
function containsBadWord(t=""){ return listProfanities(t).length>0; }
function censorText(text=""){ let out=String(text); BAD_PATTERNS.forEach(re=>{ out=out.replace(re,(m,grp)=>m.replace(grp,"*".repeat(grp.length))); }); return out; }
function levenshtein(a,b){ const m=a.length,n=b.length; if(!m) return n; if(!n) return m; const dp=Array.from({length:n+1},(_,j)=>j);
  for(let i=1;i<=m;i++){ let prev=dp[0],cur=i; dp[0]=i; for(let j=1;j<=n;j++){ const tmp=dp[j]; dp[j]=a[i-1]===b[j-1]?prev:1+Math.min(prev,cur,tmp); prev=tmp; cur=dp[j]; } } return dp[n]; }

// 3) Context euphemism (grape→rape) with intent + human target
...
function contextEuphemismGate(text=""){
  const s=String(text);
  if (/\bgrapes?\b/i.test(s) && /\b(juice|wine|seedless|raisin|raisins|fruit|bunch|vineyard|snack|harvest|farm|grapes?)\b/i.test(s)) return {ok:true};
  const hasGrape=GRAPE_FUZZY.test(s), hasIntent=INTENT_VERBS.test(canon(s)), hasHuman=HUMAN_TARGETS.test(s);
...
// 4) Perspective thresholds + call (server holds key)
const PERSPECTIVE_THRESHOLDS={ TOXICITY:0.80, SEVERE_TOXICITY:0.70, PROFANITY:0.75, INSULT:0.75, IDENTITY_ATTACK:0.65, THREAT:0.60, SEXUALLY_EXPLICIT:0.65 };
async function scoreWithPerspective(text){
  ...
  return (res && res.ok && res.top) ? res.top : null;
}

// 5) Hybrid gate
async function hybridProfanityGate(text){
  const local=listProfanities(text); if (local.length) return {ok:false,source:'local',reason:`Profanity detected: ${local[0]}`};
  const euphemism=contextEuphemismGate(text); if (!euphemism.ok) return euphemism;
  const top=await scoreWithPerspective(text); if (!top) return {ok:true};
  const limit=PERSPECTIVE_THRESHOLDS[top.name] ?? 1.0;
  return (top.score>=limit) ? {ok:false,source:'perspective',reason:`${top.name.toLowerCase()} ${top.score.toFixed(2)}`} : {ok:true};
}
...
Object.defineProperty(window,'hybridProfanityGate',{writable:false});
...
async function uploadMedia(kind /* 'avatar' | 'cover' */) {
  ...
  try {
    ...
    const { profile: p } = await apiJSON("/api/profile/update_media.php", {
      method: "POST",
      body: fd
    });
    ...
    if (kind === "avatar") {
      ...
      if (el && p?.profile_picture) el.style.backgroundImage = `url('${mediaUrl(p.profile_picture)}')`;

      // also refresh the composer avatar if present
      ...
      if (el && p?.cover_picture) el.style.backgroundImage = `url('${mediaUrl(p.cover_picture)}')`;
    }
  } catch (err) {
...
li.addEventListener('click', () => {
  const base  = (typeof PROJECT_BASE !== 'undefined' ? PROJECT_BASE.replace(/\/+$/,'') : '');
  const role  = (window.ME?.role || '').toLowerCase();
  const page  = role === 'adviser' ? 'Adviser-UserProfile.html' : 'UserProfile.html';
  location.href = `${base}/${page}?user_id=${encodeURIComponent(u.id)}`;
});
...
// Infinite scroll via IntersectionObserver (creates a sentinel + loader)
// Infinite scroll via IntersectionObserver (sentinel after #feedList)
{
  ...
  let loader = document.getElementById('feedLoading');
  if (!loader) {
    loader = document.createElement('div');
    loader.id = 'feedLoading';
    loader.className = 'feed-loading';
    loader.textContent = 'Loading more…';
    loader.hidden = true;
    sentinel.after(loader);
  }
// End-of-feed message (hidden by default)
let endcap = document.getElementById('feedEnd');
if (!endcap) {
  endcap = document.createElement('div');
  endcap.id = 'feedEnd';
  endcap.className = 'feed-end';
  endcap.textContent = 'More posts coming soon ✨';
  endcap.hidden = true;
  loader.after(endcap);
}
  ...
  const io = new IntersectionObserver(async (entries) => {
    const ent = entries[0];
    if (!ent.isIntersecting || FEED_LOADING || !FEED_HAS_MORE) return;
    // Show "end of feed" when there’s nothing else
    ...
    loader.hidden = false;
    try {
      await window.loadMyPosts({ append: true });
    } finally {
      loader.hidden = true;
    }
  }, { root: null, rootMargin: '900px 0px 0px 0px', threshold: 0 });
  

  io.observe(sentinel);

  // Prefill if the page is short
  (async function fillIfShort(max = 4) {
    for (let i = 0; i < max; i++) {
      const el = document.documentElement;
      if ((el.scrollHeight - el.clientHeight) > 800) break;
      if (FEED_LOADING || !FEED_HAS_MORE) break;
      loader.hidden = false;
      await window.loadMyPosts({ append: true });
      loader.hidden = true;
    }
...
if (avatarEditBtn) avatarEditBtn.addEventListener("click", () => uploadMedia("avatar"));
  ...
  // Keep Create Post above the feed, no matter what loaded first.
(function ensureComposerAboveFeed(){
  const post = document.getElementById('createPost');
  const feed = document.getElementById('feedList');
  if (post && feed && post.nextElementSibling !== feed) {
    feed.parentNode.insertBefore(post, feed);
  }
})();
...
try {
  const j = targetId
    ? await apiJSON(`/api/profile/get_public.php?id=${encodeURIComponent(targetId)}`)
    : await apiJSON(`/api/profile/get.php`);
  p = j.profile || {};
} catch {
  // if public profile 404s (or any error), just load *my* profile
  const j2 = await apiJSON(`/api/profile/get.php`);
  p = j2.profile || {};
}
    ...
    const nameEl = document.getElementById("user-name");
    if (cover)  cover.style.backgroundImage  = p.cover_picture   ? `url('${mediaUrl(p.cover_picture)}')`   : "";
    if (avatar) avatar.style.backgroundImage = p.profile_picture ? `url('${mediaUrl(p.profile_picture)}')` : "";
...
const composerPfp = document.querySelector('#createPost .post-avatar');
if (composerPfp) {
  const url = p.profile_picture ? mediaUrl(p.profile_picture) : (window.DEFAULT_PFP || DEFAULT_PFP);
  if (composerPfp.tagName === 'IMG') composerPfp.src = url;
  else composerPfp.style.backgroundImage = `url('${url}')`;
  // harden against future 404s
  if (composerPfp.tagName === 'IMG') {
    composerPfp.onerror = () => { composerPfp.src = (window.DEFAULT_PFP || DEFAULT_PFP); };
  }
}

if (new URLSearchParams(location.search).has('user_id')) {
  applyReadOnlyIfOther(p.id);
}
...
function buildPostCard(p) {
  ...
  // Stamp an owner id so the kebab toggle can fall back reliably
card.dataset.ownerId = String(
  p.user_id ?? p.author_id ?? p.owner_id ?? p.student_id ?? p.adviser_id ?? ""
);
...
function togglePostKebabForOwner(postEl, post) {
  ...
  // Which profile page (if any) am I on?
  const getTarget = (typeof window.getProfileTarget === 'function') ? window.getProfileTarget : () => null;
  const targetId     = String(getTarget() || "");
  const onMyProfile = !!(targetId && targetId === meId);

  // We only “force show” on my own profile page; everywhere else we show only if I own the post.
  ...
  // Final decision
  const isMine = onMyProfile ? true : (ownerId && ownerId === meId);
...
// Single source of truth for pagination on the newsfeed
window.loadMyPosts = async function ({ before = null, append = false } = {}) {
  ...
  // use explicit arg first, then global cursor
const cursor = append ? (before ?? FEED_CURSOR) : null;
  if (cursor != null && cursor !== '') qs.set('before', String(cursor));

  const url = `/api/posts/feed.php?${qs.toString()}&_=${Date.now()}`;
  console.debug('[feed] fetch', { cursor, FEED_CURSOR, url });

  try {
    if (!append) {
      document.getElementById('feedEnd')?.setAttribute('hidden','');
document.getElementById('feedLoading')?.setAttribute('hidden','');
    ...
    // work with or without has_more from server
    FEED_HAS_MORE = (j.has_more ?? j.more ?? (items.length >= LIMIT)) ? true : false;
    // Now that FEED_HAS_MORE is final, sync UI + stop observing if done
const endEl  = document.getElementById('feedEnd');
const loadEl = document.getElementById('feedLoading');
if (endEl) endEl.hidden = !!FEED_HAS_MORE;
if (loadEl && !FEED_HAS_MORE) loadEl.hidden = true;
if (!FEED_HAS_MORE) window.__feedIO?.disconnect();
...
/*async function submitPost(e) {
  e.preventDefault();
  ensureTemplates?.(); // if present in your module
  ...
  pendingCard = renderPost(optimistic);
  pendingCard.dataset.pending = '1';
  feed.prepend(pendingCard);
  pendingCard.insertAdjacentHTML('beforeend', '<div class="post-pending">Posting…</div>');
...
try {
  ...
  if (typeof containsBadWord === 'function' && containsBadWord(caption)) {
    if (typeof submit !== 'undefined' && submit) submit.disabled = true;
    if (typeof captionWarn !== 'undefined' && captionWarn) {
      captionWarn.hidden = false;
      captionWarn.textContent = 'Profanity detected. Please revise.';
    }
    window.__posting = false;
    return;
  }
  ...
  try {
    ...
    if (created && pendingEl) {
      ...
      if (typeof togglePostKebabForOwner === 'function') togglePostKebabForOwner(realEl, created);
...
cleanSubmit.addEventListener('click', submitPost);
  ...
  const updateReady = () => {
    ...
    cleanSubmit.disabled = !(hasText || hasFile);
     const caption = (input?.value || '').trim();
