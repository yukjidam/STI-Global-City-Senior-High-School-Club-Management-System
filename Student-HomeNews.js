...
// === Notifications (LIVE) — loader (student) ===
(() => {
  ...
  const src = (BASE || '') + '/notif_student.js';
  if (!document.querySelector('script[src$="notif_student.js"]')) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (news) {
    news.href = 'Student-Newsfeed.html';
    news.addEventListener('click', (e) => {
      // ensure we navigate even if drawer JS intercepts
      e.preventDefault();
      window.location.href = 'Student-Newsfeed.html';
    });
  }

  if (club) {
    club.href = 'Student-ClubDetails.html';
    club.addEventListener('click', (e) => {
      e.preventDefault();
      window.location.href = 'Student-ClubDetails.html';
    });
  }
});


// Link mobile bottom tabs → pages / settings modal
(function wireMobileBottomTabs(){
  const root = document.getElementById('mobileBottomTabs');
  if (!root) return;

  // NEWSFEED → Student-Newsfeed.html
  const mbNews = root.querySelector('.mbtab[aria-label="Newsfeed"]');
  if (mbNews && !mbNews.__wired) {
    mbNews.__wired = true;
    mbNews.setAttribute('href', 'Student-Newsfeed.html');
    mbNews.addEventListener('click', (e) => {
      e.preventDefault();
      window.location.href = 'Student-Newsfeed.html';
    });
  }

  // SETTINGS → reuse your existing Settings open logic
  const mbSettings = root.querySelector('.mbtab[aria-label="Settings"]');
  if (mbSettings && !mbSettings.__wired) {
    mbSettings.__wired = true;
    mbSettings.addEventListener('click', (e) => {
      e.preventDefault();
      // Prefer to trigger one of the existing "openers" you already wired:
      const trigger =
        document.querySelector('.open-settings') ||
        document.querySelector('.side-nav__item[data-label="Settings"]') ||
        Array.from(document.querySelectorAll('.dropdown-menu a'))
          .find(a => (a.textContent || '').trim().toLowerCase() === 'settings');

      if (trigger) {
        trigger.click(); // uses your existing modal open + prefill code
        return;
      }

      // Fallback: open the overlay directly (no prefetch)
      const overlay = document.getElementById('settingsModalOverlay');
      if (overlay) {
        overlay.classList.add('open');
        overlay.setAttribute('aria-hidden','false');
  ...
  // (Optional) Ensure Club also navigates on mobile, if you want it explicit:
  const mbClub = root.querySelector('.mbtab[aria-label="Club"]');
  if (mbClub && !mbClub.__wired) {
    mbClub.__wired = true;
    mbClub.setAttribute('href', 'Student-ListOfClub.html');
    mbClub.addEventListener('click', (e) => {
      e.preventDefault();
